...
- **Error analysis** (`notebooks/errors.ipynb`): on residual-error lines (post-correction did not fix), uses `difflib.SequenceMatcher` to count three edit types between corrected text and ground truth — substitutions (model wrote X, gt was Y), deletions (model over-generated), insertions (model dropped chars). Top-20 of each surface domain-specific patterns (long-s confusions, punctuation drops, whitespace differences). Three cherry-picked failure cases with the actual handwriting image alongside the texts: (a) confidently wrong (high TrOCR logprob but high gt_cer — exactly what the agreement signal exists to catch), (b) over-correction (Claude rewrote heavily but result is still wrong), (c) Claude returned content for the wrong line ID (~0.3% alignment-failure rate, documented honestly). Closes with four concrete v2 directions derived from the patterns.
